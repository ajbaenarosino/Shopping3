...
class Index {
private:
    Pair _entries[MAXEVENT]; // array of Pair
     ...
     */
    void setIOnWhich(const int val);
public:
     ...
     */
    Index(const int onBrand); // UsersIndex onBrand=0  o BrandType onBrand=1, defaults to 0
     ...
     */
    void build(const EventSet &evSet, const int onBrand);
     ...
     */
    const Pair at(const int pos) const;
 ...
 * @return An Index which is a subset of the current one in which all events
