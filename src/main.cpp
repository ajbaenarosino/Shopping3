 ...
 */
void errorBreak(const int errorcode, const string &errordata = "");
...
int main(int argc, char** argv) {
    ...
    bool existing_args[] = {false, false, false};
    for (int i = 1; i < argc && noargs == false; i++) {
        ...
        if (curr_arg == "-noargs") {
            ...
            existing_args[1] = true;
        } else if (curr_arg == "-index-by") { // -report-by Brand|UserID|Type|Datetime"  "-output <output_file>"  "-display Framed|Fancy
            i++;
            curr_arg = argv[i];
            ...
            existing_args[2] = true;
        } else if (curr_arg == "-report-by") {
            i++;
            curr_arg = argv[i];
            bool valid = false;
            for (int j = 0; j < sizeof (VALID_FIELDS) / sizeof (string) && valid == false; j++) {
                if (curr_arg == VALID_FIELDS[j]) {
                    valid = true;
                }
            }
            if (valid) {
            ...
            i++;
            curr_arg = argv[i];
            if (curr_arg != "Framed" && curr_arg != "Fancy") {
                errorBreak(ERROR_ARGUMENTS);
            } else {
                display = curr_arg;
            }
    ...
    if (noargs) {
        ...
        for (int i = 0; i < sizeof (existing_args) / sizeof (bool); i++) {
            if (existing_args[i] == false) {
                errorBreak(ERROR_ARGUMENTS);
            }
        }
    }
    ...
    indx.build(originalES, int_indexby);
    ...
    EventSet filteredES = extractEventSet(originalES, indx);
    ...
    for (int i = 0; i < n_values; i++) {
        Index aux_indx = rawFilterIndex(filteredES, indx, reportby, values[i]);
        traffic[i] = aux_indx.size();
        amount[i] = sumPrice(filteredES, aux_indx);
    }
    ...
    Index filtered_indx = rawFilterIndex(filteredES, indx, reportby, values[max_pos]);
    ...
    if (display == "Fancy") {
        cout << hitsData.showFancyReportV() << endl;
        cout << amountData.showFancyReportV() << endl;
    } else if (display == "Framed") {
        cout << hitsData.showFramedReportV() << endl;
        cout << amountData.showFramedReportV() << endl;
    } else {
        cout << hitsData.showPlainReportV() << endl;
        cout << amountData.showPlainReportV() << endl;
    }

    cout << "Element with the highest traffic: " << values[max_pos] << endl;
    cout << "Number of events with this value: " << traffic[max_pos] << endl;
    ...
    if (ofilename != "" && ofilename != "empty") {
        if (ofile.is_open()) {
            write(ofile, filteredES, filtered_indx);
        } else {
...
void computeActivity(const EventSet &evSet, int arrayA[]) {
    for (int i = 0; i < evSet.size(); i++) {
        int week_day = evSet.at(i).getDateTime().weekDay();
        arrayA[week_day]++;
    }
}
